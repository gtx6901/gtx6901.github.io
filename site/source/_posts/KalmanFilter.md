...
需要注意的是，过程噪声描述的是系统在真实演化过程中受到的随机扰动，它的具体取值对滤波器而言是未知的。因此，在进行先验预测时，不能把 $w_k$ 直接写进估计值公式。
...
这表示：滤波器只能利用上一时刻的后验估计、系统模型以及已知控制输入，对当前状态进行预测。
...
代入上式并展开，在假设 $w_k$ 与上一时刻估计误差独立的前提下，可得：
$$
...
$$
我们注意到，式中的 $cov(\tilde{x}_{k|k-1})$ 正是先验误差协方差矩阵。按照前面的定义，
$$
P_{k|k-1}=E[\tilde{x}_{k|k-1}\tilde{x}_{k|k-1}^T]
$$
因此这里可以直接记为 $P_{k|k-1}$。
